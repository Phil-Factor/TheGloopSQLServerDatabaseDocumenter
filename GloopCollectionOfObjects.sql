DECLARE @JSON NVARCHAR(MAX) =
          (
          SELECT Name, Type, Description, Referencing, [Referenced by],
            AggregatedObjects AS [contains]
            FROM
              ...
              SELECT object.object_id, --needed to find the child objects!
                String_Escape(
Object_Schema_Name(object.object_id) + '.' + object.name, 'json'
)               AS "Name",
                Lower(Replace(object.type_desc, '_', ' ')) + ' ('
                + RTrim(object.type) + ')' AS "Type", -- the type of object
-- SQL Prompt formatting off
      ...
      Json_Query(
         (SELECT '["'+
                ...
                ,'", "')  +'"]' AS referenced
               FROM sys.sql_expression_dependencies
               WHERE referencing_id = object.object_id)) AS [Referencing],
